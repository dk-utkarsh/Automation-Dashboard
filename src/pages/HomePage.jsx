    ...
    <div className="px-8 py-10 md:px-16">
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          ...
          <h2 className="text-4xl font-extrabold text-[#2B6CB0] tracking-tight">Departmental Hub</h2>
        </div>
        ...
        {departments.map((dept) => (
          ...
          >
            <div className="flex justify-between">
              <span className="material-symbols-outlined text-[#2B6CB0] text-3xl transition-transform group-hover:scale-110">
                {iconMap[dept.slug] || "folder"}
              </span>
              <span className="text-[#2B6CB0] font-bold text-xl">{dept.tool_count || 0}</span>
            </div>
            <div>
              <h3 className="text-lg font-bold text-[#2B6CB0] mt-6">{dept.name}</h3>
      ...
      {departments.length > 0 && (
        <div className="bg-white rounded-2xl p-10 border border-slate-200 flex flex-col md:flex-row gap-12 items-center shadow-sm">
          <div className="flex-1">
            <h4 className="text-2xl font-bold text-[#2B6CB0] mb-4">Enterprise Insights</h4>
          ...
          <div className="w-full md:w-1/3 relative aspect-square rounded-xl overflow-hidden shadow-2xl border-4 border-white bg-[#2B6CB0]">
            ...
            <div className="absolute bottom-6 left-6 right-6 bg-white/90 backdrop-blur-md p-4 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-[#2B6CB0] uppercase tracking-tight">Live Systems Operational</span>
              </div>
            </div>
          </div>
        </div>
