...
export default function ToolForm() {
  ...
  useEffect(() => {
    if (!isEdit) return;
    api.getTool(toolId).then(tool => setForm({ name: tool.name, description: tool.description || "", url: tool.url, icon: tool.icon, status: tool.status, tags: (tool.tags || []).join(", ") }))
  ...
  return (
    <div className="max-w-lg">
      <Link to={`/admin/departments/${departmentId}/tools`} className="text-slate-500 hover:text-[#3B8FCF] text-sm font-medium mb-6 inline-flex items-center gap-1">
        <span className="material-symbols-outlined text-lg">arrow_back</span> Back to Tools
      </Link>
      <h2 className="text-2xl font-extrabold text-[#3B8FCF] mb-6">{isEdit ? "Edit Tool" : "New Tool"}</h2>
